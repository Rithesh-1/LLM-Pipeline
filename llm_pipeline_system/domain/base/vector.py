...
from llm_pipeline_system.application.networks.embeddings import EmbeddingModelSingleton
from llm_pipeline_system.domain.exceptions import ImproperlyConfigured
from llm_pipeline_system.domain.types import DataCategory
from llm_pipeline_system.infrastructure.db.qdrant import connection
...
class VectorBaseDocument(BaseModel, Generic[T], ABC):
    ...
    def bulk_find(cls: Type[T], limit: int = 10, **kwargs) -> tuple[list[T], UUID | None]:
        try:
            ...
            logger.error(
                f"Failed to search documents in '{cls.get_collection_name()}'. Qdrant client returned error: {e}"
            )
