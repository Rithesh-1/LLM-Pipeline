...
# Optional unsloth import for training (requires CUDA)
try:
    from unsloth import PatchDPOTrainer
    PatchDPOTrainer()
    UNSLOTH_AVAILABLE = True
except (ImportError, AssertionError) as e:
    print(f"Warning: Unsloth not available (likely no CUDA): {e}")
...
# Conditional trl import for training
try:
    from trl import DPOConfig, DPOTrainer, SFTTrainer
    TRL_AVAILABLE = True
except ImportError as e:
    print(f"Warning: TRL not available: {e}")
    DPOConfig = None
    DPOTrainer = None
    SFTTrainer = None
    TRL_AVAILABLE = False

# Conditional unsloth imports
...
def load_model(
    ...
    if not UNSLOTH_AVAILABLE:
        raise RuntimeError("Unsloth is not available. Training requires CUDA support.")
...
def finetune(
    ...
    if not TRL_AVAILABLE:
        raise RuntimeError("TRL is not available. Training requires compatible transformers version.")
...
def inference(
    ...
    if not UNSLOTH_AVAILABLE:
        raise RuntimeError("Unsloth is not available. Inference requires CUDA support.")
...
def save_model(model: Any, tokenizer: Any, output_dir: str, push_to_hub: bool = False, repo_id: Optional[str] = None):
    if not UNSLOTH_AVAILABLE:
        raise RuntimeError("Unsloth is not available. Model saving requires CUDA support.")
